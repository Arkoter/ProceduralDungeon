...
public class ProceduralDungeons extends JavaPlugin {
    ...
    private void cleanupOldDungeons() {
        long threshold = configManager.getCleanupThreshold();
        if (threshold > 0) {
            getServer().getScheduler().runTaskAsynchronously(this, () -> {
                dungeonData.cleanupOldDungeons(threshold);
                getLogger().info("Nettoyage des anciens donjons effectué");
            });
        }
    }
